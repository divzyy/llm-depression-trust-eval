...
if __name__ == "__main__":
    ...
    model = os.environ.get("MODEL_NAME", "alibayram/medgemma:27b")  # Set MODEL_NAME env var in SLURM to change model
    print(f"DEBUG: MODEL_NAME = {model}")

    model_tag = model.replace(":", "_").replace("/", "_")
    model_display_name = model.split("/")[-1].split(":")[0].capitalize()

    OUTPUT_DIR = f"{ROOT}/analysis_output/LLMs Study/{model_display_name}/quan_few_shot"
                        ...
                        while retry_count < max_retries and not participant_processed:
                            ...
                            try:
                                with timeout(600):  # 10 minutes of processing, afterwards the attempt is re-tried. Done to handle infinite looping
                                    transcript_path = f"{DAIC}/transcripts/{participant_id}_TRANSCRIPT.csv"
