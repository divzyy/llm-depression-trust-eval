...
#SBATCH --output=logs/llm_job1_qual_%j.out
#SBATCH --error=logs/llm_job1_qual_%j.err
...
if [ "$FREE_MEM" -lt "17000" ]; then
    ...
    sbatch "$AIPSY_ROOT/LLMs Study/job1qualfeedback_llm.sh"
    exit 1
fi
...
python "LLMs Study/qual_assessment_llm.py"
echo "Step 1 done: $(date)"

echo ""
echo "=== Step 2: Judge + Feedback Loop ==="
python "LLMs Study/feedback_loop_llm.py"
...
FEEDBACK_OUT="$AIPSY_ROOT/analysis_output/LLMs Study/${MODEL_DISPLAY}/qual/feedback_assessments_v2.csv"
if [ ! -f "$QUAL_OUT" ]; then
...
echo "Model: $MODEL_NAME"
echo "Output files:"
echo "  analysis_output/LLMs Study/${MODEL_DISPLAY}/qual/qual_assessment_${MODEL_TAG}_v2.csv"
echo "  analysis_output/LLMs Study/${MODEL_DISPLAY}/qual/feedback_assessments_v2.csv"
echo "  analysis_output/LLMs Study/${MODEL_DISPLAY}/qual/feedback_evaluations_v2.csv"
echo "========================================"
