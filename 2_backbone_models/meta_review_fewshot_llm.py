...
def main():
    ...
    output_list = []
    qual_output_dir = os.path.join(rootdir, f"analysis_output/LLMs Study/{model_display_name}/qual")
    quan_few_shot_dir = os.path.join(rootdir, f"analysis_output/LLMs Study/{model_display_name}/quan_few_shot")
    os.makedirs(qual_output_dir, exist_ok=True)
    output_filename = os.path.join(qual_output_dir, "meta_review_fewshot_test_v2.csv")

    qual_df = pd.read_csv(os.path.join(qual_output_dir, f"qual_assessment_{model_tag}_v2.csv"))

    quan_list = load_jsonl(
        os.path.join(
            quan_few_shot_dir,
    ...
    for participant_id in participant_id_list:
        ...
        if not os.path.exists(transcript_path):
            print(f"Transcript missing for participant {participant_id}, skipping")
            continue
        ...
        rows_q = qual_df.loc[qual_df['participant_id'] == participant_id, 'qualitative_assessment']
        if rows_q.empty:
            print(f'Skipping {participant_id}: no qualitative assessment'); continue
        qual_output = rows_q.iloc[0]
        qual_output = re.sub(r'^```[a-zA-Z]*\s*\n?', '', qual_output)
        qual_output = re.sub(r'\n?```\s*$', '', qual_output)
