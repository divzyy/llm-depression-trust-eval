...
def main():
    ...
    quan_dir = os.path.join(rootdir, f"analysis_output/LLMs Study/{model_display_name}/quan")
    os.makedirs(qual_output_dir, exist_ok=True)
    output_filename = os.path.join(qual_output_dir, "meta_review_zeroshot_test_v2.csv")
    ...
    quan_list = load_jsonl(
        os.path.join(
            quan_dir,
            "results_zero_shot_test41_detailed.jsonl"
        )
    )
