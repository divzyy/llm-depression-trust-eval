...
#SBATCH --output=logs/llm_job3_fewshot_%j.out
#SBATCH --error=logs/llm_job3_fewshot_%j.err
...
mkdir -p $HOME/logs
mkdir -p "$AIPSY_ROOT/analysis_output/LLMs Study/${MODEL_DISPLAY}/quan_few_shot"
...
if [ "$FREE_MEM" -lt "17000" ]; then
    ...
    sbatch "$AIPSY_ROOT/LLMs Study/job3fewmeta_llm.sh"
    exit 1
fi
...
echo ""
if [ -f "$PICKLE" ]; then
    echo "=== Step 1: Pickle already exists, skipping generation ==="
    echo "Using: $PICKLE"
else
...
python "LLMs Study/embedding_batch_script_llm.py" \
...
FEWSHOT="$AIPSY_ROOT/analysis_output/LLMs Study/${MODEL_DISPLAY}/quan_few_shot/ids_test_chunk_8_step_2_dim_4096_examples_2_embedding_results_analysis_1.jsonl"
if [ ! -f "$FEWSHOT" ]; then
...
python "LLMs Study/meta_review_fewshot_llm.py"
echo "Step 3 done: $(date)"

META_OUT="$AIPSY_ROOT/analysis_output/LLMs Study/${MODEL_DISPLAY}/qual/meta_review_fewshot_test_v2.csv"
if [ ! -f "$META_OUT" ]; then
...
echo "Model: $MODEL_NAME"
echo "Output files:"
echo "  agents/chunk_8_step_2_participant_embedded_transcripts_${MODEL_DISPLAY,,}.pkl"
echo "  analysis_output/LLMs Study/${MODEL_DISPLAY}/quan_few_shot/ids_test_chunk_8_step_2_dim_4096_examples_2_embedding_results_analysis_1.jsonl"
echo "  analysis_output/LLMs Study/${MODEL_DISPLAY}/qual/meta_review_fewshot_test_v2.csv"
echo "========================================"
