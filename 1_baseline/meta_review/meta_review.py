...
def main():
    ...
    output_list = []
    output_filename = f"{ROOT}/analysis_output/qual/meta_review_fewshot_test_v2.csv"

    qual_df = pd.read_csv(os.path.join(ROOT, "analysis_output/qual/qual_assessment_GEMMA_v2.csv"))

    quan_list = load_jsonl(
        os.path.join(
            ROOT,
            "analysis_output/quan_gemma_few_shot/ids_test_chunk_8_step_2_dim_4096_examples_2_embedding_results_analysis_1.jsonl"
        )
    )
