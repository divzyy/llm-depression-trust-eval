...
def main():
    ...
    VARIANT_BASE = f"{ROOT}/analysis_output/VariantD/rate_{RATE}/seed_{SEED}"
    OUTPUT_DIR = os.path.join(VARIANT_BASE, "meta")
    os.makedirs(OUTPUT_DIR, exist_ok=True)

    output_list = []
    output_filename = os.path.join(OUTPUT_DIR, "meta_review_zeroshot_test_v2.csv")

    qual_df = pd.read_csv(os.path.join(VARIANT_BASE, "qual", "qual_assessment_GEMMA_v2.csv"))

    quan_list = load_jsonl(
        os.path.join(VARIANT_BASE, "quan_zero_shot", "results_zero_shot_test41_detailed.jsonl")
    )
    ...
    for participant_id in participant_id_list:
        transcript_path = f"{VARIANT_TRANSCRIPT_DIR}/{participant_id}_TRANSCRIPT.csv"
