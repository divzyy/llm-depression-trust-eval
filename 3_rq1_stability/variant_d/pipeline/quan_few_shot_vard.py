...
def get_embedding(text, model="qwen3-embedding:8b-q8_0", dim=None):
    ...
    """
    BASE_URL = f"http://{OLLAMA_NODE}:{OLLAMA_PORT}/api/embeddings"
...
if __name__ == "__main__":
    ...
    OLLAMA_PORT = os.environ.get("OLLAMA_PORT", "11434")   # NEW: unique port for gpu-short jobs
    BASE_URL = f"http://{OLLAMA_NODE}:{OLLAMA_PORT}/api/chat"
    model = "gemma3:27b"  # alibayram/medgemma:27b

    VARIANT_TRANSCRIPT_DIR = f"{ROOT}/rq1_perturbations/variant_d/rate_{args.rate}/seed_{args.seed}"
    OUTPUT_DIR = f"{ROOT}/analysis_output/VariantD/rate_{args.rate}/seed_{args.seed}/quan_few_shot"
                        ...
                        while retry_count < max_retries and not participant_processed:
                            ...
                            try:
                                with timeout(600):  # 10 minutes of processing, afterwards the attempt is re-tried. Done to handle infinite looping
                                    transcript_path = fr"{VARIANT_TRANSCRIPT_DIR}/{participant_id}_TRANSCRIPT.csv"
