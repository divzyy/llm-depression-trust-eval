#!/bin/bash
#SBATCH --job-name=vard_gen_5
#SBATCH --output=logs/vard_gen_5_%j.out
#SBATCH --error=logs/vard_gen_5_%j.err
...
mkdir -p $HOME/logs
mkdir -p $AIPSY_ROOT/rq1_perturbations/variant_d/rate_5
...
curl -s -m 1800 -X POST http://localhost:11434/api/chat   -d '{"model":"gemma3:27b","messages":[{"role":"user","content":"say hi"}],"stream":false}'   > /tmp/warmup_5.json
...
# --- Run generation for rate 5% ---
export OLLAMA_NODE=localhost
cd $AIPSY_ROOT

echo ""
echo "=== Generating transcripts: rate=5%, seeds=1-5 ==="
python 3_rq1_stability/variant_d/generate_variant_d.py --rate 5
echo "Generation done: $(date)"

echo ""
echo "Checking transcript counts:"
for seed in 1 2 3 4 5; do
    DIR=$AIPSY_ROOT/rq1_perturbations/variant_d/rate_5/seed_${seed}
    COUNT=$(ls $DIR/*.csv 2>/dev/null | wc -l)
    echo "  rate_5/seed_${seed}: ${COUNT}/41 transcripts"
done

LOG=$AIPSY_ROOT/rq1_perturbations/variant_d/rate_5/generation_log.csv
if [ -f "$LOG" ]; then
...
echo "Output: rq1_perturbations/variant_d/rate_5/"
echo "========================================"
