...
def get_embedding(text, model="qwen3-embedding:8b-q8_0", dim=None):
    ...
    if response.status_code == 200:
        ...
        if dim is not None and str(dim).strip() not in ('', 'default', 'None'):
...
def process_evidence_for_references(evidence_dict, participant_embedded_transcripts, phq8_ground_truths, dim=None, top_k=2):
    """
...
if __name__ == "__main__":
    ...
    parser.add_argument('--eval_split', choices=['val', 'test'], default='test',
                        help='Use validation split for hyperparameter tuning or test split for final replication')
    ...
    OLLAMA_NODE = args.ollama_node if args.ollama_node else os.environ.get("OLLAMA_NODE", "localhost")
    BASE_URL = f"http://{OLLAMA_NODE}:11434/api/chat"
    model = "gemma3:27b"  # alibayram/medgemma:27b

    VARIANT = os.environ.get("VARIANT", "ALL")

    OUTPUT_DIR = f"{ROOT}/analysis_output/VariantA/{VARIANT}/quan_few_shot"
    LOG_FILE = os.path.join(OUTPUT_DIR, "embedding_log_file.txt")

    os.makedirs(OUTPUT_DIR, exist_ok=True)

    log_message(f"RQ1 VARIANT: {VARIANT}")
    log_message("="*80)
    ...
    log_message(f"Evaluation split: {args.eval_split}")
    ...
    # Override test_ids with paper's exact 41 subjects for final replication only
    if args.eval_split == 'test':
        test_ids = [316, 319, 330, 339, 345, 357, 362, 367, 370, 375, 377, 379, 383,
                    385, 386, 389, 390, 393, 409, 413, 417, 422, 423, 427, 428, 430,
                    436, 441, 445, 447, 449, 451, 455, 456, 459, 468, 472, 484, 485,
                    487, 489]
        log_message(f"NOTE: test_ids overridden with paper's exact 41 subjects for replication")

    # RQ1 OVERRIDE — restrict to 13 selected subjects for Variant A
    test_ids = [316, 330, 339, 357, 362, 375, 385, 386, 389, 390, 413, 422, 449]
    log_message(f"RQ1 OVERRIDE: test_ids restricted to {len(test_ids)} subjects for Variant A {VARIANT}")

    eval_ids = val_ids if args.eval_split == 'val' else test_ids
    ...
    log_message(f"\nEvaluation IDs ({args.eval_split}, {len(eval_ids)}): {eval_ids}")
    ...
    for chunk_step in chunk_steps:
        for dim in dims:
            pickle_file = f"{ROOT}/agents/{chunk_step}_participant_embedded_transcripts.pkl"
            ...
            for examples_num in examples_nums:
                ...
                for run_num in range(1, args.num_runs + 1):
                    ...
                    log_message(f"chunk_step: {chunk_step}, dim: {dim if dim else 'default'}, examples_num: {examples_num}, run: {run_num}")
                    log_message("="*80)

                    if dim:
                        JSONL_FILE = os.path.join(
                            OUTPUT_DIR,
                            f"ids_{args.eval_split}_{chunk_step}_dim_{dim}_examples_{examples_num}_embedding_results_analysis_{run_num}.jsonl"
                        )
                    else:
                        JSONL_FILE = os.path.join(
                            OUTPUT_DIR,
                            f"ids_{args.eval_split}_{chunk_step}_examples_{examples_num}_embedding_results_analysis_{run_num}.jsonl"
                        )

                    log_message(f"Output file: {JSONL_FILE}")
                    log_message(f"Total participants to process: {len(eval_ids)}")
                    ...
                    for idx, participant_id in enumerate(eval_ids):
                        ...
                        while retry_count < max_retries and not participant_processed:
                            ...
                            if retry_count > 1:
                                ...
                                log_message(f"[{chunk_step}, {examples_num}, run {run_num}] Processing participant {participant_id} ({idx+1}/{len(eval_ids)})...")
                            
                            try:
                                with timeout(600):  # 10 minutes of processing, afterwards the attempt is re-tried. Done to handle infinite looping
                                    transcript_path = f"{ROOT}/rq1_perturbations/variant_a/{VARIANT}/{participant_id}_TRANSCRIPT.csv"
                                    current_transcript = _load_transcript(transcript_path)
                                ...
                                error_msg = f"Timeout: Participant {participant_id} took longer than 10 minutes (attempt {retry_count}/{max_retries})"
                    ...
                    log_message(f"\n[{chunk_step}, dim={dim if dim else 'default'}, {examples_num}, run {run_num}, split={args.eval_split}] Combination completed. Successful: {successful}, Failed: {failed}")
                    log_message(f"[{chunk_step}, dim={dim if dim else 'default'}, {examples_num}, run {run_num}, split={args.eval_split}] Results saved to: {JSONL_FILE}")
