...
echo "========================================"
echo "RQ1 Variant A — ${VARIANT} — Few-shot Quan + Meta"
...
if [ "$FREE_MEM" -lt "17000" ]; then
    ...
    sbatch --export=ALL,VARIANT=${VARIANT} --job-name=${VARIANT} \
        $AIPSY_ROOT/slurm/varA_job3_fsmetaPHQ.sh
    exit 1
fi
...
echo ""
echo "=== Step 1: Few-shot Quantitative Assessment — ${VARIANT} ==="
python 3_rq1_stability/variant_a_phq/rq1_embedding_batch_PHQ.py \
...
FS_OUT=$AIPSY_ROOT/analysis_output/VariantA/${VARIANT}/quan_few_shot/ids_test_chunk_8_step_2_dim_4096_examples_2_embedding_results_analysis_1.jsonl
if [ ! -f "$FS_OUT" ]; then
...
echo ""
echo "=== Step 2: Meta Review — Few-shot (${VARIANT}) ==="
python 3_rq1_stability/variant_a_phq/rq1_meta_review_fs_PHQ.py
echo "Step 2 done: $(date)"

META_OUT=$AIPSY_ROOT/analysis_output/VariantA/${VARIANT}/meta/meta_review_fs_${VARIANT}.csv
if [ ! -f "$META_OUT" ]; then
...
echo "Output files:"
echo "  analysis_output/VariantA/${VARIANT}/quan_few_shot/ids_test_chunk_8_step_2_dim_4096_examples_2_embedding_results_analysis_1.jsonl"
echo "  analysis_output/VariantA/${VARIANT}/meta/meta_review_fs_${VARIANT}.csv"
echo "========================================"
