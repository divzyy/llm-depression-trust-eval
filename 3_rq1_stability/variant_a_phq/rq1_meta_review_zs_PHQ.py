...
def main():
    ...
    output_list = []
    output_filename = f"{ROOT}/analysis_output/VariantA/{VARIANT}/meta/meta_review_zs_{VARIANT}.csv"

    qual_df = pd.read_csv(f"{ROOT}/analysis_output/VariantA/{VARIANT}/qual/qual_assessment_{VARIANT}.csv")

    quan_list = load_jsonl(
        f"{ROOT}/analysis_output/VariantA/{VARIANT}/quan_zero_shot/results_zs_{VARIANT}_detailed.jsonl"
    )
