#!/bin/bash
#SBATCH --job-name=varA_job3_few
#SBATCH --output=logs/rq1_varA/job3_few_%j.out
#SBATCH --error=logs/rq1_varA/job3_few_%j.err
...
echo "========================================"
echo "RQ1 Variant A — Job 3: Few-shot Quan + Meta (13 subjects)"
...
if [ ! -f "$PICKLE" ]; then
    ...
    echo "Run baseline job3 first to generate the pickle."
    exit 1
fi
...
if [ "$FREE_MEM" -lt "17000" ]; then
    ...
    sbatch $AIPSY_ROOT/slurm/varA_job3_fsmetaAll.sh
    exit 1
fi
...
echo ""
echo "=== Step 1: Few-shot Quantitative Assessment (13 varA subjects) ==="
python 3_rq1_stability/variant_a_all/rq1_embedding_batch_ALL.py \
...
FS_OUT=$AIPSY_ROOT/analysis_output/VariantA/All/quan_few_shot/ids_test_chunk_8_step_2_dim_4096_examples_2_embedding_results_analysis_1.jsonl
if [ ! -f "$FS_OUT" ]; then
    echo "ERROR: Few-shot output not found: $FS_OUT"
    kill $OLLAMA_PID
    exit 1
fi
echo "Few-shot output confirmed: $FS_OUT"

echo ""
echo "=== Step 2: Meta Review — Few-shot (varA) ==="
python 3_rq1_stability/variant_a_all/rq1_meta_review_fs_ALL.py
echo "Step 2 done: $(date)"

META_OUT=$AIPSY_ROOT/analysis_output/VariantA/All/meta/meta_review_fs_ALL.csv
if [ ! -f "$META_OUT" ]; then
...
echo "Output files:"
echo "  analysis_output/VariantA/All/quan_few_shot/ids_test_chunk_8_step_2_dim_4096_examples_2_embedding_results_analysis_1.jsonl"
echo "  analysis_output/VariantA/All/meta/meta_review_fs_ALL.csv"
echo "========================================"
