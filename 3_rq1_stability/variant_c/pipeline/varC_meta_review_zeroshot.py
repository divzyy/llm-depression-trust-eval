# RQ1 Variant C — Meta Review (zero-shot quantitative input)
import os
import csv
import json
import requests
...
def main():
    ...
    output_filename = os.path.join(OUTPUT_DIR, "meta_review_zeroshot_varC.csv")

    qual_df = pd.read_csv(os.path.join(
        rootdir, "analysis_output/VariantC/qual/qual_assessment_varC.csv"
    ))

    quan_list = load_jsonl(os.path.join(
        rootdir, "analysis_output/VariantC/quan/results_zero_shot_varC_detailed.jsonl"
    ))

    seen = {}
    for item in quan_list:
        seen[item["participant_id"]] = item
    quan_list = list(seen.values())
    print(f"NOTE: Excluded from varC pipeline — zero substitutions: [357, 385, 423]")
    print(f"Loaded {len(quan_list)} unique participants from ZS JSONL")
    ...
    for participant_id in participant_id_list:
        transcript_path = (
            f"{ROOT}/rq1_perturbations/variant_c"
            f"/{participant_id}_TRANSCRIPT_varC.csv"
        )
        ...
        try:
            ...
            print(f"{participant_id}: error parsing response")
            continue
    ...
    print(f"\nZero-shot meta review saved: {output_filename}")
    print(f"Subjects processed: {len(output_list)}")
