#!/bin/bash
#SBATCH --job-name=dyn_v2_subtle
#SBATCH --output=logs/dyn_v2_subtle_%j.out
#SBATCH --error=logs/dyn_v2_subtle_%j.err
...
for attempt in $(seq 1 10); do
    ...
    echo "  waiting 60s for the GPU..."; sleep 60
done
if [ "$FREE_MEM" -lt "17000" ]; then
    echo "ERROR: GPU still busy after 10 minutes. The chained job will retry."; exit 1
fi
...
curl -s -m 1800 -X POST http://localhost:11434/api/chat \
  ...
  > /tmp/warmup_dyn_v2s.json
echo "Warmup:"; cat /tmp/warmup_dyn_v2s.json | python3 -c "import sys,json; print(json.load(sys.stdin).get('message',{}).get('content','ERROR'))" 2>/dev/null
$OLLAMA_BIN ps
...
export ATTACKS_ONLY=T_Subtle,Q_Subtle
export RUN_TAG=v2subtle

cd $AIPSY_ROOT
echo ""
echo "=== Adaptive attacks v2, subtle only (validator fixed) ==="
python 4_rq2_injection/run_dynamic_vpi_experiment_v2.py

kill $OLLAMA_PID 2>/dev/null || true
echo ""
echo "Job complete: $(date)"
echo "Results:  rq2_proinject/vpi_dynamic_results_v2subtle.csv"
echo "Payloads: rq2_proinject/dynamic_payloads_used_v2subtle.csv"
